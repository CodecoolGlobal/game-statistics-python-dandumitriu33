...
def print_all():
    selected_year = input('Enter the year to see if there are games from then on the list: ')
    selected_genre = input('Enter a genre to count how many games it has on the list: ')
    selected_title = input('Enter a title to see what number it is on the list: ')
    print('The total number of games is %d.' % count_games(THE_FILE))
    print('Are there games from {} on the list?'.format(selected_year), decide(THE_FILE, selected_year))
    ...
    print('{} is number {} on the list.'.format(selected_title, get_line_number_by_title(THE_FILE, selected_title)))
